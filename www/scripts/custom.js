...
$( document ).ready(function() {
    ...
    var n = this, 
     ...
     function refresh(companydata , loginType){
        var data = {
                'user':loginType === 'Boss'?companydata['boss_user'] : companydata['sales_user'],
                'password':loginType === 'Boss'?companydata['boss_password'] : companydata['sales_password'] ,
                'type':loginType,
            ...
            $.ajax({
                ...
                success: function(data, textStatus){    
                    ...
                    window.location.href = loginType === 'Boss'? "home.html" : "homesales.html";

                    
                },
                error:function(xhr,textStatus,errorThrown){
                    // alert(this.url+' An error occurred! ' + ( errorThrown ? errorThrown : xhr.status ));
                    alert('Ensure your internet connection is active!');
                }
            });
     }
    function init_template(){//Class is vital to run AJAX Pages
        ...
        $('#errorqty').hide();
        $('#successqty').hide();
        // if(window.location.toString().indexOf( 'sales.html' ) > 0){
            ...
            for( var a in d ){
                ...
                if( a === 'todaysales'){
                    ...
                    for(var i =0; i<d[a].length;i++){
                        rows +='<tr><td>'+r[i]['stock_name']+'</td><td>'+r[i]['quantity']+'</td><td>'+parseFloat(r[i]['total_price']).formatMoney(2,'.',',')+'</td></tr>';
                        totalprice += parseFloat(r[i]['total_price']);
                        totalcost += ( parseFloat(r[i]['quantity']) * parseFloat(r[i]['cost_price']) );
                    }

                    $('tbody#salescontent').html(rows);
                    var profit = totalprice - totalcost;
                    $('#todaysales').html(totalprice.formatMoney(2,'.',','));
                    $('#todayprofit').html(profit.formatMoney(2,'.',','));
                }
                ...
                if(a === 'notifications'){
                    ...
                    for( var i=0;i<n.length;i++){
                        nots += '<a href="#" class="reminder reminder-check-square"><strong><span style="font-size:11px;">'+n[i]['stock_name']+' is almost out. You have just '+n[i]['quantity']+' left!</span></strong><em><i class="fa fa-clock-o"></i>'+date+'</em></a>  ';
                    }

                    $('#stocknotification').html(nots);
                }
                if( a === 'sales'){
                    var r2 = d[a];
                    var rows2 = '';
                    ...
                    var totalcost2 = 0;
                    for(var i =0; i<d[a].length;i++){
                        rows2 +='<tr><td>'+r2[i]['stock_name']+'</td><td>'+r2[i]['quantity']+'</td><td>'+parseFloat(r2[i]['total_price']).formatMoney(2,'.',',')+'</td></tr>';
                        totalprice2 += parseFloat(r2[i]['total_price']);
                        totalcost2 += ( parseFloat(r2[i]['quantity']) * parseFloat(r2[i]['cost_price']) );
                    }

                    $('tbody#allsales').html(rows2);
                    var profit2 = totalprice2 - totalcost2;
                    $('#totalsales').html(totalprice2.formatMoney(2,'.',','));
                    $('#totalprofit').html(profit2.formatMoney(2,'.',','));
                }
                if( a === 'stocks'){
                    var makesalesstock = '';
                    var r3 = d[a];
                    ...
                    var totalcost3 = 0;
                    for(var i =0; i<d[a].length;i++){
                        makesalesstock += '<option>'+r3[i]['stock_name']+'</option>';
                        rows3 +='<tr><td>'+r3[i]['stock_name']+'</td><td>'+r3[i]['quantity']+'</td><td>'+parseFloat(r3[i]['cost_price']).formatMoney(2,'.',',')+'</td><td>'+parseFloat(r3[i]['sell_price']).formatMoney(2,'.',',')+'</td></tr>';
                        totalprice3 += parseFloat(r3[i]['total_price']);
                        totalcost3 += ( parseFloat(r3[i]['quantity']) * parseFloat(r3[i]['cost_price']) );
                    }

                    $('tbody#allstock').html(rows3);
                    $('select#items').html(makesalesstock);
                    $('#costPrice').val(r3[0]['cost_price']);
                    $('#price').val(r3[0]['sell_price']);
                }
            }
        }
        ...
        $('#items').change(function(){
            ...
            for(var i =0; i<stocks.length;i++){
                if( $(this).val() === stocks[i]['stock_name']){
                   $('#price').val(stocks[i]['sell_price']);
                   $('#costPrice').val(stocks[i]['cost_price']);
                }
            }
        ...
        $('#price').focusout(function(){
            ...
            $('#errorqty').hide();
            $('#successqty').hide();
            if( parseFloat($(this).val()) < parseFloat($('#costPrice').val()) ){
                
                $('#errorprice').show();
                $(this).val($('#costPrice').val());
            }else{
                if(saleserror > 0){
                    saleserror -= 1;
                }
        ...
        $('#quantity').keyup(function(){
            ...
            if ( $(this).val() != '' && $(this).val() !== null ){
                for(var i =0; i<stocks.length;i++){
                    if( $('#items').val() === stocks[i]['stock_name']){
                        if( parseFloat($(this).val()) > parseFloat(stocks[i]['quantity']) ){
                            saleserror += 1;
                            $('#errorqty').show();
                            $('#successqty').hide();
                       }else{
        ...
        $('#salesBtn').click(function(event){
            ...
            $('#errorqty').hide();
            $('#successqty').hide();
            if( saleserror <= 0  && parseFloat($('#price').val()) >= parseFloat($('#costPrice').val())){
                $('#successqty').hide();
                ...
                var total = parseFloat(sellp) * parseFloat(qty) ;
                var stockid = '';
                var availqty = '';
                for(var i =0; i<stocks.length;i++){
                    if( stock === stocks[i]['stock_name']){
                        stockid = stocks[i]['stock_id'];
                        availqty = stocks[i]['quantity'];;
                    }
                }
                ...
                var url1 = 'http://api.bosstrader.com.ng/v1/makesales';
                    ...
                    success: function(data, textStatus){    
                        for(var a in data){
                            ...
                            if(a === 'success'){
                                $("#successsale").show();
                                setTimeout(function(){
                                    // refresh(companydata[0],'Sales Person');
                                }, 3000);
                            }
                        }
